...
class USER
{
	...
	public function register($usrcode,$role,$uname,$email,$upass,$code)
	{
		try
		{
			$password = md5($upass);
		
			$stmt = $this->conn->prepare("INSERT INTO tbl_im_users(userCode,role_role_id,userName,userEmail,userPass,tokenCode)
			                                             VALUES(:user_code,:role_id,:user_name, :user_mail, :user_pass, :active_code)");
	...
	public function login($email,$upass)
	{
		try
		{
			...
			$stmt = $this->conn->prepare("SELECT * FROM tbl_im_users WHERE userCode=:email_id");
